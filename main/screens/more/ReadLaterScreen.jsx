...
export default function ReadLaterScreen({ route }) {
    ...
    const screens = params.screens || context.screens;
    ...
    const renderError = () => {
        if (error?.isAuth || error?.message === t('screen_bookmarks_error_not_logged_in')) {
            return (
                <View
                    style={[
    ...
    if (loading) {
        return (
            <LoadingSpinner currentTheme={currentTheme} message={t('screen_read_later_loading')} />
        );
    }
