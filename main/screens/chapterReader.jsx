    ...
    return (
        <View style={styles.pullIndicatorContainer}>
            <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
                <Circle cx={size / 2} cy={size / 2} r={size / 2} fill={theme.cardBackground} />
                <Circle
...
const ChapterReader = ({
        ...
        AsyncStorage.getItem('WordReplaceRules').then(rulesString => {
            ...
            for (const rule of rules) {
                ...
                try {
                    ...
                    console.warn(`Invalid word replacer rule "${rule.title || match}":`, e);
                }
            }
        ...
        const manageHistory = async () => {
            ...
            try {
                ...
                if (
                    ...
                    console.log(
                        `History updated for work ${workId}. End chapter is now ${currentChapterIndex}`,
                    );
                } else {
                    const newEntry = {
                        workId,
                    ...
                    console.log(
                        `New history entry created for work ${workId}, chapter ${currentChapterIndex}`,
                    );
                }
            } catch (error) {
                    ...
                    case 'scroll': {
                        ...
                        if (!isIncognitoMode) {
                            ...
                            progressSaveTimeoutRef.current = setTimeout(() => {
                                ...
                                if (Math.abs(progress - lastSavedProgressRef.current) > 0.01) {
                                    // 1% change threshold
    ...
    ${
        jsonSettings &&
        !jsonSettings.allowSelectingText &&
  ...
  `;
    ...
    const renderTopBar = () => (
        ...
        >
            <View style={styles.titleContainer}>
                ...
                <Text
                    style={[styles.chapterTitle, { color: currentTheme.secondaryTextColor }]}
                    numberOfLines={1}
                >
                    {chapterTitle}
                </Text>
                {isIncognitoMode && (
                    <Text style={[styles.incognitoIndicator, { color: currentTheme.primaryColor }]}>
    ...
    const renderBottomBar = () => (
        ...
        >
            {hasPreviousChapter && (
                <TouchableOpacity
                    style={[styles.navButton, { backgroundColor: currentTheme.cardBackground }]}
            ...
            >
                <View style={styles.progressInfo}>
                    ...
                    <Text style={[styles.chapterInfo, { color: currentTheme.secondaryTextColor }]}>
    ...
    const renderCommentsButton = () => (
        <Animated.View style={{ opacity: fadeAnim }} pointerEvents={barsVisible ? 'auto' : 'none'}>
            <TouchableOpacity
                style={[styles.floatingButton, { backgroundColor: currentTheme.cardBackground }]}
        ...
        return (
            <View style={[styles.pullContainer, { opacity }]} pointerEvents="none">
                ...
                <Text style={[styles.pullText, { color: currentTheme.textColor }]}>
                    {progress < 1 ? t('reader_pull_up_next') : t('reader_release_next')}
                </Text>
            </View>
        );
    };
            ...
            <View style={[styles.container, { backgroundColor: currentTheme.backgroundColor }]}>
                ...
                >
                    <CommentsScreen
                        ...
                        singleChapter={totalChapters < 2}
                        workOrChapterId={chapterID || workId}
