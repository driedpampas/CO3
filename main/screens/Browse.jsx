...
const ClearIcon = ({ color, size }) => <Icon name={'close'} style={{ color: color }} size={size} />;
        ...
        const applyPreset = async () => {
            try {
                ...
                if (p.relationships?.length)
                    filters['work_search[relationship_names]'] = itemsToString(p.relationships);
                if (p.additionalTags?.length)
                    filters['work_search[freeform_names]'] = itemsToString(p.additionalTags);
    ...
    useEffect(() => {
        ...
        const runTempSearch = async () => {
            try {
                ...
                if (preset.wordCount) filters['work_search[word_count]'] = preset.wordCount;
                if (preset.language) filters['work_search[language_id]'] = preset.language;
                ...
                if (preset.characters?.length)
                    filters['work_search[character_names]'] = itemsToString(preset.characters);
                ...
                if (preset.additionalTags?.length)
                    filters['work_search[freeform_names]'] = itemsToString(preset.additionalTags);
                ...
                if (preset.comments) filters['work_search[comments_count]'] = preset.comments;
                if (preset.bookmarks) filters['work_search[bookmarks_count]'] = preset.bookmarks;
                ...
                setAppliedFilters(filters);
                setHasFilters(Object.keys(filters).length > 0 || !!preset.canonicalTagName);
            } catch (err) {
