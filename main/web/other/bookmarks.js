...
export async function fetchBookmarks(page, username, pseud, noWebview = false) {
    ...
    try {
        ...
        let workElements = Array.from(olElements[0].getElementsByTagName('li')).filter(li =>
            li.getAttribute('class')?.includes('bookmark blurb'),
        );

        if (workElements.length === 0) {
            workElements = Array.from(olElements[1].getElementsByTagName('li')).filter(li =>
                li.getAttribute('class')?.includes('bookmark blurb'),
            );
        }
    ...
    try {
        ...
        if (pseudTagMatch) {
            ...
            const selectMatch = html.match(
                /<select[^>]*name="bookmark\[pseud_id\]"[^>]*>[\s\S]*?<option[^>]*value="([^"]+)"[^>]*selected/i,
            );
