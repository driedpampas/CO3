...
export async function checkTagCanonical(tagName) {
    ...
    const canonicalLink = allLinks.find(a =>
        (a.getAttribute('href') || '').includes('/faq/glossary#canonicaldef'),
    );
    ...
    if (!isCanonical) {
        ...
        for (const p of allParagraphs) {
            ...
            if (
                text.toLowerCase().includes('synonym of') ||
                text.toLowerCase().includes('merger of')
            ) {
    ...
    console.log(
        `Tag "${tagName}": exists=${exists}, isCanonical=${isCanonical}, category=${category}, canonicalFor=${canonicalFor}`,
    );
