    ...
    try {
        ...
        for (let i = coms.length - 1; i > -1; i--) {
            if (coms[i].childNodes.length < 2) {
                addComment(
                    new Comment({
                        id: i,
                        isBanner: true,
                        html: coms[i].childNodes[0]
                            .toString()
                            .replace('/comments/', 'https://archiveofourown.org/comments/'),
                    }),
                    'next',
                );
                continue;
            ...
            if (!authorIsDeleted) {
                ...
                const authorURL = aOrSpan.getAttribute('href')?.split('/');
            ...
            let html, content;
            if (preferHTML)
                html = Array.from(blockquote.childNodes)
                    .map(v => v.toString())
                    .join('');
            else content = getElementText(blockquote);

            const actions = coms[i]
                .getElementsByClassName('actions', false)[0]
                .getElementsByTagName('a');
            let parentId = null;
            for (let a = 0; a < actions.length; a++) {
                if (getElementText(actions[a]).toLowerCase() === 'parent')
                    parentId = actions[a].getAttribute('href').split('/comments/')[1];
            }
