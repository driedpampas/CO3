...
async function scrapeUserPage(url, username) {
    ...
    const avatar = Array.from(doc.getElementsByTagName('img')).filter(
        img => img?.getAttribute('class') === 'icon',
    )[0];
    const bio = Array.from(doc.getElementsByTagName('blockquote')).filter(
        a =>
            a.getAttribute('class') === `userstuff` &&
            a.parentNode.getAttribute('id') !== 'admin-banner',
    )[0];

    const meta = Array.from(doc.getElementsByTagName('dl')).filter(
        a => a.getAttribute('class') === `meta`,
    )[0];
    console.log(meta);
