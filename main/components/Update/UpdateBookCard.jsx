...
const UpdateBookCard = ({ update, workDAO, theme, onPress }) => {
    ...
    useEffect(() => {
        ...
        const checkStatus = async () => {
            ...
            if (isMounted.current) {
                ...
                setHasFailed(
                    failedList.some(f => String(f.chapterId) === String(update.chapterID)),
                );
    ...
    const handleDownloadPress = async () => {
        ...
        if (hasFailed) {
            ...
            const newList = failedList.filter(
                f => String(f.chapterId) !== String(update.chapterID),
            );
    ...
    const renderDownloadIcon = () => {
        ...
        if (isDownloadedFile && showDelete)
            return <Icon name="delete" size={24} color={theme.warningBackground} />;
        if (isDownloadedFile)
            return <Icon name="check-circle" size={24} color={theme.primaryColor} />;
    ...
    let warningImage =
        imageMappings.warningStatus[work.warnings] || imageMappings.warningStatus.default;
