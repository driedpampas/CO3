...
export class WorkDAO {
    ...
    async countWorks() {
        const [results] = await this.db.executeSql('SELECT COUNT(*) as count FROM works');
    ...
    async _syncTags(workId, tagNames) {
        ...
        for (const name of tagNames) {
            let [tagResult] = await this.db.executeSql('SELECT id FROM tags WHERE name = ?', [
                name,
            ]);
            let tagId;

            if (tagResult.rows.length === 0) {
                [tagResult] = await this.db.executeSql('INSERT INTO tags (name) VALUES (?)', [
                    name,
                ]);
    ...
    async _syncWarnings(workId, warningNames) {
        ...
        for (const name of warningNames) {
            let [warnResult] = await this.db.executeSql('SELECT id FROM warnings WHERE name = ?', [
                name,
            ]);
            let warningId;

            if (warnResult.rows.length === 0) {
                [warnResult] = await this.db.executeSql('INSERT INTO warnings (name) VALUES (?)', [
                    name,
                ]);
