...
export class UpdateDAO {
    ...
    async getAll() {
        const [results] = await this.db.executeSql('SELECT * FROM updates ORDER BY date DESC');
    ...
    async getTotalCount() {
        const [results] = await this.db.executeSql('SELECT COUNT(*) as count FROM updates');
