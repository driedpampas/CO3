...
export class KudoHistoryDAO {
     ...
     */
    async get(workId) {
        const [results] = await this.db.executeSql('SELECT * FROM kudo_history WHERE workId = ?', [
            workId,
        ]);
     ...
     */
    async getAll() {
        const [results] = await this.db.executeSql('SELECT * FROM kudo_history ORDER BY date DESC');
        return Array.from(
    ...
    async getTotalHistoryCount() {
        const [results] = await this.db.executeSql('SELECT COUNT(*) as count FROM kudo_history');
        return results.rows.item(0).count;
    }
